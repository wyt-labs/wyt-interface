...
const ChatDialog = (
  ...
  const { isSignedIn, walletAddress } = useWallet();
  ...
  const overviewSubItems = [
    // { id: 'Tokenomics', isSelected: false },
    // { id: 'Profitability', isSelected: false },
    { id: 'Team', isSelected: false },
    { id: 'Funding', isSelected: false },
    { id: 'Exchanges', isSelected: false },
    { id: 'All', isSelected: false },
  ];

  const chartSubItems = [
    {
      id: 'New tokens',
      desc: 'Daily new tokens on Pump.fun',
      isSelected: false,
    },
    {
      id: 'Time distribution',
      desc: 'Time distribution of new token launches on Pump.fun',
      isSelected: false,
    },
    {
      id: 'Transactions',
      desc: 'Pump.fun daily transactions',
      isSelected: false,
    },
    {
      id: 'Top traders',
      desc: 'Pump.fun top traders',
      isSelected: false,
    },
  ];
  ...
  const ask = async (e: any) => {
    ...
    if (onChatResult.code === 0) {
      ...
      if (isNewChat) {
        // @ts-ignore
        history.push({
          pathname: isUniswap ? '/uniswap-chat' : '/chat',
          query: {
            id,
          },
        });
  ...
  const getOverView = (content: any) => {
    if (content.type && content?.type === 'project_info') {
      const related_links = (
        content.project_info?.project?.overview?.related_links || []
      ).filter((item) => item.type === 'Official Website');
      if (related_links.length > 0) {
        return (
          <div
            onClick={() => {
              window.open(related_links[0].link);
            }}
            className="flex rounded-lg px-4 py-2 ml-auto bg-gray-50 hover:bg-gray-200 cursor-pointer"
          >
            <div className="text-gray-500 text-sm mt-0.5">Website</div>
            <MoreIcon className="ml-3 mt-0.5" />
          </div>
        );
      }
  ...
  return (
    <>
      <div className="w-full flex flex-col pb-10 h-full pt-4">
        {isUniswap && !isLoading ? (
          <div
